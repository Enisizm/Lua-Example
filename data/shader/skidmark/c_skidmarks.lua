...
addEventHandler( "onClientStartShader", resourceRoot,
	function(verbose)
		...
		if not myShader then
			...
			if verbose then outputChatBox( "Using technique " .. tec ) end
...
function toggleSkidmarksShaderResource(bool)
	...
	if bEnabled == bool then return false end
	bEnabled = bool
	if bEnabled then
		enableSkidmarksShader()
	else
		disableSkidmarksShader()
	end
end

function enableSkidmarksShader()
	-- Create shader
	myShader, tec = dxCreateShader ( "data/shader/skidmark/skidmarks.fx" )
	engineApplyShaderToWorldTexture ( myShader, "particleskid" )
	selectPreset(3)
	panel.donator.state[2] = true
end

function disableSkidmarksShader()
	destroyElement(myShader)
	panel.donator.state[2] = false
end
